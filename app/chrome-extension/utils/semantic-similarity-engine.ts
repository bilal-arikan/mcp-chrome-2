...
async function getCachedModelData(modelUrl: string): Promise<ArrayBuffer> {
  ...
  try {
    ...
    // 3. Get data and store it in cache
    ...
    // If the fetch fails, clean up the possibly incomplete cache entry
...
export class SemanticSimilarityEngine {
  ...
  private async _doInitializeWithProgress(
    ...
    try {
      ...
      // 🛠️ Prevent infinite loop: if already in the offscreen document, force direct Worker mode
      ...
      if (this.useOffscreen) {
        ...
        // Ensure the config object is serialized correctly, explicitly setting all properties
        const configToSend = {
          modelIdentifier: this.config.modelIdentifier,
          localModelPathPrefix: this.config.localModelPathPrefix,
    ...
    try {
      ...
      if (this.useOffscreen) {
        ...
        const response = await chrome.runtime.sendMessage({
          target: 'offscreen',
          type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_INIT,
          config: serializedConfig, // Use the original config, don't force-modify useLocalFiles
        });
        ...
        if (this.config.useLocalFiles) {
          // For WXT, assets under the public directory are located at the root path at runtime
          // Use the model identifier directly; transformers.js automatically adds the /models/ prefix
        ...
        // For models that don't need token_type_ids, set this explicitly in the tokenizer config
  ...
  private async _initializeDirectWorkerWithProgress(
    ...
    // Use the progress callback feature of transformers.js 2.17+
    ...
    try {
      ...
      // If the progress callback isn't supported, fall back to the standard approach
      console.log(
  ...
  public async warmupModel(): Promise<void> {
    ...
    // More representative warmup texts, covering different lengths and languages
    const warmupTexts = [
      // Short text
      'Hello',
      'Hi there',
      ...
      // Non-Latin sample to warm up the multilingual model across scripts
      '你好世界，这是一个测试。',
      // Long text
    ...
    try {
      ...
      // Keep warmup results, don't clear the cache
  ...
  private async _tokenizeText(text: string | string[]): Promise<TokenizedOutput> {
    ...
    if (typeof text === 'string') {
      ...
      // For models that don't need token_type_ids, explicitly set return_token_type_ids to false
      if (!this.config.requiresTokenTypeIds) {
    ...
    // For batch text, process directly (batches usually aren't repeated)
    const startTime = performance.now();
    const tokenizerOptions: any = {
      padding: true,
      truncation: true,
  ...
  private _extractEmbeddingFromWorkerOutput(
    ...
    // Use the memory pool to get the embedding array
  ...
  public async getEmbeddingsBatch(
    ...
    if (this.useOffscreen) {
      ...
      // Place the results back in their positions and cache them
  ...
  public cosineSimilarity(vecA: Float32Array, vecB: Float32Array): number {
    ...
    // Use the SIMD-optimized version (if available)
    if (this.useSIMD && this.simdMath) {
      try {
        // The SIMD version is async, but to keep interface compatibility we need a sync version
        // Here we fall back to the JavaScript version, or we could consider refactoring to async

/**
 * @fileoverview Record-Replay V3 service-level E2E integration test
 * @description
 * Verifies the complete V3 flow: RPC → enqueue → schedule → run → complete
 *
 * Tests use:
 * - Real IndexedDB storage (fake-indexeddb)
 * - service-level RPC (directly calling internal handler, avoiding Port mock)
 */
...
describe('V3 service-level E2E', () => {
  ...
  describe('Event streaming', () => {
    ...
    it('subscribe with runId filter only receives events for that run', async () => {
      ...
      // Subscribe to receive only runId1 events (but runId1 has already completed)
  ...
  describe('Control plane', () => {
    ...
    it('cancel queued run removes it from queue', async () => {
      // Create a new harness that does not auto-start the scheduler
      await h.dispose();
  ...
  describe('Recovery', () => {
    it('orphan running lease is requeued and run can complete', async () => {
      // Stop the current harness, create a new one without starting the scheduler
      await h.dispose();
      ...
      // The queue item should return to the queued state
      ...
      // Start the scheduler; the Run should be able to continue executing
      h.scheduler.start();
    ...
    it('adopts orphan paused items', async () => {
      ...
      // The queue item should still be paused, but the owner is switched to the new one
    ...
    it('cleans terminal runs left in queue', async () => {
      ...
      // Simulate a crash scenario: Run completed but the queue item was not cleaned up
      await h.storage.queue.enqueue({ id: runId, flowId: flow.id, priority: 0 });
      await h.storage.queue.markRunning(runId, 'owner-old', Date.now());

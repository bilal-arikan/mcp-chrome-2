...
export function useBuilderStore(initial?: FlowV2 | null) {
  ...
  function setNodePosition(id: string, x: number, y: number) {
    ...
    // Not pushed to the history stack to avoid frequent records; recorded on user-triggered actions (connect/add/delete, etc.).
  }
  ...
  // Fallback layout: layering + barycenter ordering (no external library)
  ...
  // - Dynamically import elkjs to avoid a persistent bundle size

 ...
 * - Supports retry, timeout, and error handling policies
 * - Supports selector candidate lists and stability scoring
 * - Supports the variable system
 * - Follows SOLID principles (interfaces extensible via declaration merging)
 */
...
export interface TimeoutPolicy {
  ms: Milliseconds;
  /** 'attempt' = timed per attempt, 'action' = timed over the whole action */
...
export interface RetryPolicy {
  ...
  /** Backoff strategy */
  backoff?: BackoffKind;
  /** Maximum interval (used for exp/linear) */
 ...
 * so external modules can extend the Action type via declaration merging (OCP-compliant)
 */
...
export interface ActionExecutionResult<T extends ActionType = ActionType> {
  ...
  error?: ActionError;
  /** Label of the next edge (used for conditional branching) */
  nextLabel?: EdgeLabel;
...
export interface ActionHandler<T extends ExecutableActionType = ExecutableActionType> {
  ...
  /** Generate an action description (for UI display) */
...
export interface Flow {
  ...
  /** Subflows (used for foreach/while/loopElements) */
  subflows?: Record<
    SubflowId,

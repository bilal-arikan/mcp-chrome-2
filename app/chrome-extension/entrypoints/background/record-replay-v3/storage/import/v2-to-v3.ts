 ...
 * @description Converts V2 format data to V3 format, supporting bidirectional conversion
 */
 ...
 * @returns Conversion result with success/failure status, data, and error/warning info
 */
 ...
 *
 * Rules:
 * 1. Exclude trigger-type nodes (these are UI nodes that don't participate in execution)
 * 2. Only count "executable node -> executable node" edges when computing in-degree (ignore edges from triggers)
 * 3. Find nodes with in-degree 0 as candidates
 * 4. If there are multiple candidates, use a stable selection rule:
 *    - Prefer the node closest to the top-left by UI coordinates (ascending x, then ascending y when x is equal)
 *    - If no UI coordinates, pick the first by lexicographic ID order
 */
function findEntryNodeId(nodes: NodeV3[], edges: EdgeV3[]): EntryNodeResult {
  const warnings: string[] = [];

  // 1. Exclude trigger nodes to get executable nodes
  ...
  // 2. Compute in-degree (only count edges between executable nodes)
  const inDegree = new Map<NodeId, number>();
  for (const node of executableNodes) {
    inDegree.set(node.id, 0);
  }
  for (const edge of edges) {
    // Ignore edges originating from non-executable nodes (e.g. triggers)
  ...
  if (rootNodes.length === 0) {
    // No in-degree 0 node means the graph contains a cycle; use the stable selector to pick a fallback
  ...
  // 4. Single root node, return directly
  if (rootNodes.length === 1) {
    return { nodeId: rootNodes[0].id, warnings };
  }

  // 5. Multiple root nodes, use the stable selection rule
 ...
 * Prefer UI coordinates (top-left first), then lexicographic ID order
 */
function selectStableRootNode(nodes: NodeV3[]): StableSelectionResult {
  ...
  if (nodesWithUi.length > 0) {
    // Sort by UI coordinates: ascending x -> ascending y -> lexicographic id (as tie-breaker)
  ...
  // No UI coordinates, use lexicographic ID order
...
function convertMetaV2ToV3(v2Meta: V2Flow['meta']): FlowV3['meta'] | undefined {
  ...
  // Return undefined if meta is an empty object
...
export function convertTriggerV2ToV3(v2Trigger: V2Trigger): ConversionResult<TriggerSpec> {
  ...
  // Build a different TriggerSpec depending on type
  ...
  switch (v2Trigger.type) {
    ...
    case 'schedule': {
      // Convert the V2 schedule to a cron expression
...
function convertScheduleToCron(schedule: V2Trigger['schedule']): string | null {
  ...
  switch (schedule.type) {
    case 'interval': {
      // Convert the interval to an approximate cron (every N minutes)
    ...
    case 'daily':
      ...
      return '0 0 * * *'; // Default to 0:00 every day

    case 'weekly': {
      // On the specified days and time each week
...
/**
 * Create a NotImplemented V2ToV3Converter (backward compatibility)

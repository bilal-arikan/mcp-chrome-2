 ...
 * @description Implements queue CRUD operations and atomic claim
 */
 ...
 * @description Implements queue persistence, including Phase 3 atomic claim
 */
  ...
  return {
    ...
    async recoverOrphanLeases(
      ...
      return withTransaction(RR_V3_STORES.QUEUE, 'readwrite', async (stores) => {
        ...
        /**
         * Scan and reclaim orphaned running items
         * @description
         * - Orphan definition: no lease or lease.ownerId !== currentOwnerId
         * - Reclaim policy: status -> queued, clear lease, preserve attempt
         */
        ...
        /**
         * Scan and adopt orphaned paused items
         * @description
         * - Orphan definition: no lease or lease.ownerId !== currentOwnerId
         * - Adoption policy: keep status=paused, update lease.ownerId to the new ownerId, renew TTL
         */
        const recoverPausedItems = (): Promise<void> =>
          new Promise<void>((resolve, reject) => {
            ...
            request.onsuccess = () => {
              ...
              // Adopt: update lease to the new ownerId, renew TTL
              const updated: RunQueueItem = {
                ...item,
                updatedAt: now,
                lease: {
        ...
        // Run sequentially: process running first, then paused

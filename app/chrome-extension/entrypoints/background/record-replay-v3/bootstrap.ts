 ...
 * - Use the global keepalive-manager singleton to avoid multiple controllers conflicting
...
/**
 * Resolve the Tab ID needed to execute a Run
 ...
 * Note: re-reads the latest RunRecord to obtain the correct startedAt
 */
async function failRun(
  ...
  // Re-fetch the latest run record to obtain the correct startedAt
...
function createDefaultRunExecutor(deps: {
  ...
  return async (item: RunQueueItem): Promise<void> => {
    ...
    try {
      ...
      // Register with RunnerRegistry for DebugController and RPC use

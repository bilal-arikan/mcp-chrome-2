 ...
 * @description
 * Implements fixed-interval triggering using chrome.alarms' periodInMinutes.
 *
 * Strategy:
 * - Each trigger maps to one repeating alarm
 * - Uses delayInMinutes so the first fire occurs after the configured interval
 */
...
export function createIntervalTriggerHandler(
  ...
  async function schedule(triggerId: TriggerId, expectedVersion: number): Promise<void> {
    ...
    try {
      // Create a repeating alarm using delayInMinutes and periodInMinutes
      // First fire after periodInMinutes, then every periodInMinutes thereafter

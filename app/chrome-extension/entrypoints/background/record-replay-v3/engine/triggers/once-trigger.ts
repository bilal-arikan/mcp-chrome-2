 ...
 * @description
 * Implements one-time scheduled triggering using chrome.alarms' when parameter.
 *
 * Behavior:
 * - Each trigger maps to one one-time alarm
 * - After firing, the trigger is automatically disabled (enabled=false) and uninstalled
 */
...
export interface OnceTriggerHandlerDeps {
  ...
  /**
   * Optional: custom method to disable the trigger
   * If not provided, the TriggerStore is updated directly
   */
...
/**
 * Parse triggerId from the alarm name
 */
...
export function createOnceTriggerHandler(
  ...
  // Lazily create the store to avoid issues in the test environment
  ...
  /**
   * Ensure the alarm event listener is active
   */

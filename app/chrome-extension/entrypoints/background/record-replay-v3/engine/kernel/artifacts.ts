 ...
 * @description Defines the capture and storage interface for artifacts such as screenshots
 */
 ...
 * @description Decides whether to capture artifacts based on the policy configuration
 */
...
export function createArtifactPolicyExecutor(service: ArtifactService): ArtifactPolicyExecutor {
  return {
    executeScreenshotPolicy: async (policy, context) => {
      ...
      // Save (if a filename is specified)
      if (context.saveAs) {

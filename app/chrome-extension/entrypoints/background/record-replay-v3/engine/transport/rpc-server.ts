...
export class RpcServer {
  ...
  private async handleListQueue(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Validate status against the allowlist
   ...
   * @note Only items with status=queued can be canceled; running/paused must use rr_v3.cancelRun
   */
  private async handleCancelQueueItem(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // 1. Check the queue item exists
    const queueItem = await this.storage.queue.get(runId);
    if (!queueItem) {
      throw new Error(`Queue item "${runId}" not found`);
    }

    // 2. Only allow canceling queued status (running/paused must use rr_v3.cancelRun)
    ...
    // 5. Publish the run.canceled event (via EventsBus to ensure broadcast)
  ...
  private async handleSaveFlow(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Check whether this is an update to an existing flow (query using the trimmed ID)
    ...
    // Save to storage (the storage layer performs secondary validation)
   ...
   * @description Deletes a Flow, first checking for linked Triggers and queued runs
   */
  private async handleDeleteFlow(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Check for queued runs (pending runs would fail after deletion)
   ...
   * @description Validates and converts input into a complete FlowV3 structure
  ...
  private normalizeFlowSpec(value: unknown, existingFlow: FlowV3 | null = null): FlowV3 {
    ...
    // Timestamps: inherit existingFlow.createdAt on update, use current time on create
    ...
    // variables validation: each item must be an object with a name field
  ...
  private normalizeTriggerSpec(value: unknown, opts: { requireId: boolean }): TriggerSpec {
    ...
    // Add kind-specific fields based on kind
    switch (kind) {
      case 'manual':

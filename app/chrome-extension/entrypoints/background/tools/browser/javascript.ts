    ...
    const response = await withTimeout(
      cdpSessionManager.withSession(tabId, CDP_SESSION_KEY, async () => {
        return (await cdpSessionManager.sendCommand(tabId, 'Runtime.evaluate', {
          ...
          returnByValue: true,
          awaitPromise: true,
          // CDP built-in timeout (ms), as a second safeguard alongside the outer withTimeout
          timeout: options.timeoutMs,
        })) as CDPEvaluateResult;
      }),
      // Outer timeout is slightly longer to give CDP some headroom to handle the timeout response

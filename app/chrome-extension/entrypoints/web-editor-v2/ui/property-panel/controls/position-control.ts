...
function createPositionIcon(position: PositionValue): SVGElement {
  ...
  switch (position) {
    case 'static':
      // Three horizontal lines representing normal document flow
      addLine(3.5, 4.5, 8);
      addLine(3.5, 7.5, 8);
      addLine(3.5, 10.5, 8);
      break;
    ...
    case 'relative': {
      // Dashed box marks the original position; solid block marks the offset position
      ...
      svg.append(ghost);
      // Solid block at the offset position
    ...
    case 'absolute':
      // Positioning reference lines (from container edge to element)
    ...
    case 'fixed': {
      // Pushpin shape representing fixed positioning
...
function createFlipXIcon(): SVGElement {
  ...
  // Right mirror block (solid represents after flip)
...
function createFlipYIcon(): SVGElement {
  ...
  // Bottom mirror block (solid represents after flip)

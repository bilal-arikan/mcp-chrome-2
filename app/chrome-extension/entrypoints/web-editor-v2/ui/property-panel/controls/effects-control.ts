...
export function createEffectsControl(options: EffectsControlOptions): DesignControl {
  ...
  // Per-element effect items cache (current editing session only)
  // Why a WeakMap:
  // 1. Hidden effects are not written to CSS (enabled=false) but must be remembered within the session for restoration
  // 2. WeakMap automatically releases memory when an element is removed, no manual cleanup needed
  // 3. Only inline style is read (not computed), so the cache only serves to preserve the user's hide actions
  ...
  if (headerActionsContainer) {
    // Place the + button on the right side of the group header (left of the chevron)
  ...
  function isEditing(): boolean {
    ...
    // Avoids overly broad focus detection that would prevent external style changes from syncing
  ...
  function closePopover(opts?: { commit?: boolean; rollback?: boolean }): void {
    ...
    // Sync once after closing to ensure currentItems matches the real inline value
    // Avoids inconsistency after the browser normalizes/corrects values
    if (wasOpen && !rollback) {
      syncFromTarget(true);
    }
  }
  ...
  // Listen on the document's capture phase so clicks outside the Effects control also close it
  ...
  // Listen for capture-phase click events on the document
  ...
  // Listen for capture-phase keyboard events on the root
